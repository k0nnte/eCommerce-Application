...
export default class Catalog {
  ...
  renderCatalog() {
    ...
    document.addEventListener('buttonClickedDell', (event) => {
      const custom = event as CustomEvent;
      const { key } = custom.detail;
      const { childNodes } = this.wrapper_Catalog;
      for (let i = 0; i < childNodes.length; i += 1) {
        ...
        if (
          ...
          if (item.childNodes[1].textContent === key) {
            (item.lastChild as HTMLButtonElement).classList.remove('btnOff');
            (item.lastChild as HTMLButtonElement).disabled = false;
          }
        }
      }
    });
